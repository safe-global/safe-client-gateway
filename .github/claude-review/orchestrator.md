...
3. Compose the comment and post it with `gh pr comment <PR_NUMBER> --repo <REPO> --body "..."` — that exact command shape is the only one the session permission rules allow. Everything that names a rule, a guide or a line is a link, so a reader can reach the standard being applied in one click. The comment MUST have exactly this shape:
