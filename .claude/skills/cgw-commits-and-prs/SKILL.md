...
PR body: `## Summary` and `## Changes` — the template's two sections, nothing more. Several other guides require their justification to appear in the PR description *text itself* rather than a commit message or a chat thread; every one of those goes in `## Summary`, and the doc's PR-body rule names the current set — read it there rather than from memory, since a guide can add one without this skill changing. Pre-commit evidence is the PR's own CI run.
