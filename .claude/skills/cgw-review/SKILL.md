---
name: cgw-review
description: Use when reviewing a diff or PR in safe-client-gateway, or when verifying your own completed work before committing or opening a PR. Covers the repo's two-part review standard - Part 1 maps the diff to the applicable guides via the AGENTS.md routing table and verifies every rule with file:line citations; Part 2 is deviation control (spec match, no scope creep, no new structural deviations, no unjustified ratchet growth, no dead code, pre-commit evidence not assertion, SPDX headers) - plus the required findings format. Triggers on "review this", "ready to commit", "before PR", "definition of done", "is this done", "check my work", "self-review".
---
