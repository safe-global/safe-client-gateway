...
- **Workspace membership and admin checks are never re-implemented.** Call `assertMember` / `assertAdmin` from `src/modules/spaces/domain/space-assert.utils.ts` in the route service with the injected `IMembersRepository`; do not write a private `assertActiveAdmin`, an inline `findOne({ role: 'ADMIN' })`, or a repository query that filters the space by the caller's membership (see cgw-remarks R-031).
