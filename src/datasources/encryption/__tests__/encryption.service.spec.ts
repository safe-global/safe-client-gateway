...
describe('EncryptionService', () => {
  ...
  describe('app restart / DEK determinism', () => {
    it('should decrypt after restart with same key material', () => {
      // Simulate first boot
      const service1 = new EncryptionService(new Map([[1, dekV1]]), 1, hmacKey);
  ...
  describe('KMS exchange (yearly key rotation)', () => {
    it('should support full migration lifecycle: v1-only → v1+v2 → v2-only', () => {
      ...
      // Step 1: v1 only
      const phase1 = new EncryptionService(new Map([[1, dekV1]]), 1, hmacKey);
      ...
      const phase3 = new EncryptionService(new Map([[2, dekV2]]), 2, hmacKey);
