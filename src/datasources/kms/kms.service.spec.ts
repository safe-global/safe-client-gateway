...
describe('KmsService', () => {
  ...
  describe('construction', () => {
    it('resolves the KMS configuration eagerly, so a misconfiguration fails at construction', () => {
      configurationService.getOrThrow.mockImplementation((key: string) => {
        throw new Error(`Missing required configuration: ${key}`);
      });

      expect(() => new KmsService(configurationService)).toThrow(
        'Missing required configuration: spaces.fieldEncryption.kms.keyId',
      );
    });
  });
  ...
  describe('encrypt', () => {
    ...
    it('omits the encryption context when none is provided', async () => {
      ...
      await target.encrypt({ plaintext: Buffer.from('data') });

      expect(
        kmsMock.commandCalls(EncryptCommand)[0].args[0].input,
      ).toStrictEqual({
        KeyId: keyId,
        Plaintext: new Uint8Array(Buffer.from('data')),
      });
    });
    ...
    it('throws when KMS returns no ciphertext', async () => {
      ...
      await expect(
        target.encrypt({ plaintext: Buffer.from('data') }),
  ...
  describe('decrypt', () => {
    it('decrypts under the configured key with the provided encryption context', async () => {
      ...
      expect(plaintext.toString('utf8')).toBe('alice@example.com');
    ...
    it('omits the encryption context when none is provided', async () => {
      const target = buildTarget();
      kmsMock.on(DecryptCommand).resolves({
        Plaintext: new Uint8Array(Buffer.from('key-material')),
      });

      await target.decrypt({ ciphertext: Buffer.from('wrapped') });

      expect(
        kmsMock.commandCalls(DecryptCommand)[0].args[0].input,
      ).toStrictEqual({
        CiphertextBlob: new Uint8Array(Buffer.from('wrapped')),
        KeyId: keyId,
      });
    });
    ...
    it('throws when KMS returns no plaintext', async () => {
      ...
      await expect(
        target.decrypt({ ciphertext: Buffer.from('wrapped') }),
