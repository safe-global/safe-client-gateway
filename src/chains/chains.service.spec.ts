...
describe('ChainsService', () => {
  ...
  const service: ChainsService = new ChainsService(configService, transactionManager);
  ...
  it('should retrieve the backbone metadata from the proper TransactionService', async () => {
    ...
    expect(transactionManager.getTransactionService).toHaveBeenCalledWith(chainId);
  });
});
