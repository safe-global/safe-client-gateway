...
export class SafesV2Service {
  ...
  private readonly zerionTestnetsEnabled: boolean;
  ...
  constructor(
    ...
    this.zerionTestnetsEnabled = configurationService.getOrThrow<boolean>(
  ...
  private async resolveZerionChainNames(
    ...
    const chains = [...chainsById.values()].filter(
      (chain): chain is Chain =>
        chain !== null && (this.zerionTestnetsEnabled || !chain.isTestnet),
    );
