...
export class PoliciesController {
  ...
  @ApiOperation({
    ...
    description:
      'Stores the `Configuration[]` behind a `requestConfiguration(root)`, which publishes only the hash on-chain, so a later `pending` read can report what the request changes. Call it before requesting the configuration on-chain: the root is recomputed from the configurations and has to match, but it is not required to exist on-chain yet. Idempotent: storing the same root again is a no-op.',
  })
  @ApiParam({
    name: 'spaceId',
    type: 'string',
