...
describe('GuardPolicyMapper', () => {
  let target: GuardPolicyMapper;
  const safe = {
    chainId: SEPOLIA,
  ...
  beforeEach(() => {
    target = new GuardPolicyMapper(mockLoggingService);
  });

  function map(
    policies: Array<PolicyIndexerSafePolicy>,
    overrides?: { transactionGuard?: Address | null },
  ) {
    return target.map({
      safe,
      policies,
  ...
  describe('kinds', () => {
    ...
    it('should skip an allowlist whose permission it does not know', () => {
      ...
      expect(map([policy])).toStrictEqual([]);
    ...
    it('should keep the readable bindings when one is not', () => {
      ...
      expect(map([readable, unreadable])).toHaveLength(1);
    });
  });
  ...
  describe('identity and enforcement', () => {
    ...
    it('should report a policy as unenforced when the safe has no guard set', () => {
      ...
      const [result] = map([policy], { transactionGuard: null });
    ...
    it('should report one item per binding, even when they share a list', () => {
      ...
      const result = map([transfer, transferFrom]);
