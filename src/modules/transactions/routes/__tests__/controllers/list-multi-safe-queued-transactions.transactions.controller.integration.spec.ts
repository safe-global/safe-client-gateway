...
describe('Multi-Safe queued transactions - Transactions Controller', () => {
  ...
  function mockNetworkForSafe(args: {
    chain: { chainId: string; transactionService: string };
    safeAddress: string;
    safe: ReturnType<typeof safeBuilder>['build'] extends () => infer R
      ? R
      : never;
    ...
    const getChainUrl = `${safeConfigUrl}/api/v1/chains/${args.chain.chainId}`;
    const getSafeUrl = `${args.chain.transactionService}/api/v1/safes/${args.safeAddress}`;
    const getMultisigTxUrl = `${args.chain.transactionService}/api/v2/safes/${args.safeAddress}/multisig-transactions/`;

    const prevImpl = networkService.get.getMockImplementation();
    networkService.get.mockImplementation(({ url }) => {
      if (url === getChainUrl) {
        return Promise.resolve({
          data: rawify(args.chain),
          status: 200,
        });
      }
      if (url === getSafeUrl) {
      ...
      if (url === getMultisigTxUrl) {
        return Promise.resolve({
          data: rawify({
            count: args.transactions.length,
            next: null,
            previous: null,
            results: args.transactions,
          }),
          status: 200,
        });
      }
      ...
      if (prevImpl) {
        return prevImpl({ url } as Parameters<typeof prevImpl>[0]);
      }
  ...
  it('should return queued transactions across multiple safes sorted by timestamp', async () => {
    ...
    await request(app.getHttpServer())
      ...
      .then(({ body }) => {
        ...
        // Both safes present
        const addresses = body.map(
          (item: { safeAddress: string }) => item.safeAddress,
        );
        ...
        expect(body[0].transaction.timestamp).toBeLessThanOrEqual(
          body[1].transaction.timestamp,
        );
      });
  });
  ...
  it('should return empty array when no safes have queued transactions', async () => {
    ...
    networkService.get.mockImplementation(({ url }) => {
      ...
      if (
        url === `${chain.transactionService}/api/v1/safes/${safeAddress}`
      ) {
  ...
  it('should gracefully handle a failing safe and return results for successful ones', async () => {
    ...
    networkService.get.mockImplementation(({ url }) => {
      ...
      if (
        url ===
        `${safeConfigUrl}/api/v1/chains/${failingChain.chainId}`
      ) {
        return Promise.resolve({
