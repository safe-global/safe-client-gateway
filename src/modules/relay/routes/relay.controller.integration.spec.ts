...
function relayerTypeForChainId(chainId: string): RelayerType | null {
  if (noFeeCampaignSupportedChainIds.includes(chainId)) {
    return RelayerType.NO_FEE_CAMPAIGN;
  }
  if (dailyLimitChainIds.includes(chainId)) {
...
describe('Relay controller', () => {
  ...
  describe.each(supportedChainIds)('Common Tests: Chain %s', (chainId) => {
    beforeEach(() => {
      if (noFeeCampaignSupportedChainIds.includes(chainId)) {
        ...
        const tokenBalance = {
          tokenAddress: noFeeConfig[Number.parseInt(chainId, 10)]
    ...
    describe('POST /v1/chains/:chainId/relay', () => {
      describe('Relayer', () => {
        ...
        describe('Safe', () => {
          describe.each(
            SAFE_VERSIONS[chainId],
          )('v%s execTransaction', (version) => {
        ...
        describe('ProxyFactory', () => {
          ...
          )('v%s createProxyWithNonce', (version) => {
  ...
  describe.each(dailyLimitChainIds)('Daily limit tests %s', (chainId) => {
    describe('Rate limiting', () => {
      ...
      it('should not rate limit the same address on different chains', async () => {
        // Pick any other chainId and configure it as DAILY_LIMIT so the GET
        // endpoint also routes through the DailyLimitRelayer.
  ...
  describe('No-fee campaign tests', () => {
    describe.each(
      noFeeCampaignSupportedChainIds,
    )('No-fee campaign tests [%s]', (chainId) => {
      const version = '1.3.0';
      ...
      it('should not relay transaction when token balance is zero', async () => {
        ...
        const tokenAddress = noFeeConfig[Number.parseInt(chainId, 10)]
          ?.safeTokenAddress as string;
      ...
      describe('Token balance-based relay limits', () => {
        ...
        describe.each(tokenBalanceScenarios)('Token balance scenarios', ({
          tokens,
          expectedLimit,
        }) => {
      ...
      describe('Gas limit handling', () => {
        ...
        describe.each(gasLimitScenarios)('$description', ({ gasLimit }) => {
          it('should handle gas limit correctly', async () => {
            ...
            const maxGasLimit =
              noFeeConfig[Number.parseInt(chainId, 10)]?.maxGasLimit;
            const actualGasLimit =
              typeof gasLimit === 'function' ? gasLimit(maxGasLimit) : gasLimit;
        ...
        it('reject tx exceeding maxGasLimit', async () => {
          ...
          const requestBody: Record<string, string> = {
            ...
            version,
            gasLimit: (
              noFeeConfig[Number.parseInt(chainId, 10)]?.maxGasLimit + 100_000
            ).toString(),
          };
  ...
  describe.each([
    ...
    '10',
    '11155111',
  ])('SafeWebAuthnSignerFactory: Chain %s', (chainId) => {
