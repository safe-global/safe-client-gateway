 ...
 * from other modules through `IEntitlementEnforcement`. The endpoint lives in
 * `EntitlementsRoutesModule`, which is what `FF_USERS` gates; the
 * webhook-facing sync, which needs the billing service's API client, in
...
@Module({
  ...
  exports: [EntitlementsService, IEntitlementEnforcement],
})
