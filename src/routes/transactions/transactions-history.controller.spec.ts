...
function getVanityAddress(address: `0x${string}`): `0x${string}` {
  ...
  const prefix = address.slice(0, 2 + VANITY_CHARS);
  const suffix = address.slice(-VANITY_CHARS);

  const vanityAddress =
    prefix +
    faker.finance.ethereumAddress().slice(2 + VANITY_CHARS, -VANITY_CHARS) +
    suffix;
            ...
            },
            {
              ...
              transaction: {
                ...
                txInfo: {
                  ...
                  recipient: {
                    // TODO: Remove checksumming after transfer schema is migrated to Zod
                    value: getAddress(transfer.to),
                    logoUri: null,
                    name: null,
                  },
                  richDecodedInfo: null,
