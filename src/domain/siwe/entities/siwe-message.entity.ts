...
export type SiweMessage = z.infer<ReturnType<typeof getSiweMessageSchema>>;
...
export function getSiweMessageSchema(maxValidityPeriodInSeconds: number) {
  return z.object({
    /**
    ...
    expirationTime: z
      ...
      .refine(
        (expirationTime) => {
          ...
          const maxExpirationTime = new Date(
            Date.now() + maxValidityPeriodInSeconds * 1_000,
          );

          return new Date(expirationTime) <= maxExpirationTime;
        },
        {
          message: `Must be within ${maxValidityPeriodInSeconds} seconds`,
        },
      ),
    /**
