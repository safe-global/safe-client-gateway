...
**Named exception — the auth module's route-layer primitives.** `AuthGuard`, `ElevationGuard` and the `@Auth()` decorator live in `src/modules/auth/routes/` and are imported from there by every module with an authenticated route (23 files). This is a deviation, and it is the one the anti-example above names for `auth.decorator` — but it is a *recorded* deviation rather than a fresh one, because the rule has no compliant target for a Nest guard: `domain/` holds interfaces and entities and must not know about `ExecutionContext`, and relocating them to `src/routes/common/` would invert the dependency, making shared infrastructure depend on a feature module's payload contract. The rule's stated rationale — keeping HTTP-layer concerns out of code that has no business depending on them — does not bite here either, since every consumer is itself a controller. Non-negotiable #9 requires `ElevationGuard` on sensitive Workspace routes, so it is imported the same way as `AuthGuard` beside it. Consolidating all three into shared infrastructure is the real fix and is its own change; until then, do not add *other* kinds of cross-module `routes/` import on the strength of this entry.
