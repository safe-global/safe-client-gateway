...
**Canonical example:** `src/modules/users/domain/email-encryption.constants.ts` documents the on-disk contract as immutable: the `kms:v1:` prefix, the `{userId, field}` encryption-context keys, and the blind-index HMAC construction. `src/modules/users/domain/email-encryption.service.ts`'s `encrypt`/`decrypt` bind ciphertext to `{ userId: String(userId), field: 'users.email' }` so a value can't be transplanted to another user or column, and `blindIndex` computes a keyed HMAC over the normalised plaintext for lookups without ever decrypting stored rows.

The pattern is implemented once today, for `users.email`; a second PII column replicates the same shape — entity-bound context, versioned prefix, keyed blind index — in its own per-field service analogous to `EmailEncryptionService`, rather than extending or parameterizing this one, whose constants are a frozen on-disk contract.
