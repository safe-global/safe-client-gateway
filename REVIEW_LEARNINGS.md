...
Historical raw review feedback that seeded the current engineering manual.
This is not the active convention guide for day-to-day coding. It preserves
recurring PR feedback so convention maintainers can trace why the active rules
exist.

This file is the historical raw review-memory layer. New convention
maintenance should use `/safe-engineering:code-conventions --compound`, which
promotes review feedback into `docs/engineering/code-conventions.md`,
`docs/engineering/pr-self-review-checklist.md`, and
`docs/engineering/review-learning-coverage.md`.
...
Convention backfill completed 2026-05-05 using closed PR review comments by
`closedAt`: 2026-02-05T00:00:00Z..2026-05-05T13:48:12Z, 127 PRs, 1054
comments. The active generalized rules live in
`docs/engineering/code-conventions.md`; coverage and evidence windows live in
`docs/engineering/review-learning-coverage.md`.
...
- **Prefer the smallest correct change.** Use the simplest code that satisfies the expected behavior, and keep the changed surface area small. Do not add files, providers, interfaces, factories, injection tokens, helpers, DTOs, or module wiring unless they clearly reduce current complexity, create a real boundary, are reused, or are needed for independent testability.
...
- **Avoid single-use DI abstractions.** NestJS providers are for real module boundaries, shared dependencies, external adapters, and independently mockable collaborators. If only one class needs a helper such as `createRemoteJWKSet(...)`, keep construction local instead of adding a factory interface, symbol, and provider wiring.
...
- **List new abstractions before opening a PR.** For every new file/provider/interface/helper, write down why direct local code was insufficient. If it is not reused, externally mocked, required by a real boundary, or reducing current complexity, inline it.
